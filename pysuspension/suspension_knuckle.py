...
class SuspensionKnuckle:
    ...
    def __init__(self, 
        ...
        """
        # Tire center position (absolute)
        self.tire_center = np.array([tire_center_x, tire_center_y, rolling_radius], dtype=float)
    ...
    def add_attachment_point(self, name: str, position: Tuple[float, float, float], 
        ...
        attachment = AttachmentPoint(name, pos_array, is_relative=True)
        self.attachment_points.append(attachment)
    
    def get_attachment_position(self, name: str, absolute: bool = True) -> np.ndarray:
        """
    ...
    def get_all_attachment_positions(self, absolute: bool = True) -> dict:
        ...
        Returns:
            Dictionary mapping attachment point names to positions
        """
        return {ap.name: self.get_attachment_position(ap.name, absolute) 
    ...
    def get_tire_contact_patch(self) -> np.ndarray:
        """
        Calculate the position of the tire contact patch (ground contact point).
        
        Returns:
            3D position of contact patch
        """
    ...
    def get_wheel_mounting_plane_center(self, absolute: bool = True) -> np.ndarray:
        """
        ...
        if absolute:
            ...
            return self.mounting_plane_center_relative.copy()
    ...
    def fit_to_attachment_targets(self, target_positions: List[np.ndarray]) -> Tuple[float, np.ndarray, np.ndarray]:
        """
        ...
        Returns:
            Tuple of (rms_error, new_tire_center, new_rotation_matrix)
        """
    ...
    def update_from_attachment_targets(self, target_positions: List[np.ndarray]) -> float:
        """
        ...
        """
        rms_error, new_center, new_rotation = self.fit_to_attachment_targets(target_positions)
...
if __name__ == "__main__":
    print("=" * 60)
    print("SUSPENSION KNUCKLE TEST")
    print("=" * 60)
    
    # Create a front left knuckle
    ...
    knuckle.add_attachment_point("upper_ball_joint", [0, 0, 0.25], relative=True)
    knuckle.add_attachment_point("lower_ball_joint", [0, 0, -0.25], relative=True)
    knuckle.add_attachment_point("tie_rod", [0.1, -0.1, 0.0], relative=True)
    
    print(knuckle)
    print("\nAttachment positions (absolute):")
    for name, pos in knuckle.get_all_attachment_positions(absolute=True).items():
        print(f"  {name}: {pos}")
    
    print("\nAttachment positions (relative):")
    for name, pos in knuckle.get_all_attachment_positions(absolute=False).items():
        print(f"  {name}: {pos}")
    
    print(f"\nTire axis (absolute): {knuckle.get_tire_axis(absolute=True)}")
    print(f"Wheel mounting plane center (absolute): {knuckle.get_wheel_mounting_plane_center(absolute=True)}")
    
    # Test fitting to new attachment positions
    print("\n--- Testing knuckle positioning from attachment targets ---")
    
    original_positions = list(knuckle.get_all_attachment_positions(absolute=True).values())
    target_positions = [pos + np.array([0.05, 0.0, -0.03]) for pos in original_positions]
    
    print(f"\nOriginal tire center: {knuckle.tire_center}")
    print(f"Original toe: {np.degrees(knuckle.toe_angle):.3f}°, camber: {np.degrees(knuckle.camber_angle):.3f}°")
    
    rms_error = knuckle.update_from_attachment_targets(target_positions)
    
    print(f"\nAfter fitting:")
    print(f"RMS error: {rms_error:.6f} m")
    print(f"New tire center: {knuckle.tire_center}")
